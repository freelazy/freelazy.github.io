      ...
      <script>
        var options = {
            ...
            setup: function (props) {
                const a = Vue.reactive({ x: 1, y: 2});
                const b = Vue.ref(4); // b.value
                import { state, state2 } from './a.js';
                import { aaa, bbb } from './c.js';
                Vue.watchEffect(() => { console.log(aaa); });
                onCreated(() => ());
                return { a, b, aaa, bbb, state, state2 };
            }
        };
        new Vue(options);
      </script>
