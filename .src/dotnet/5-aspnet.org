...
=IServiceCollection=:
- 表示服务的结合。它主要用来注册、保存 Service。它实现了 IList 接口，是可迭代的
...
=IServiceProvider=:
- 通过 =IServiceCollection.BuildProvider()= 的方法生成
- 其核心方法是 =GetService(serviceType)=, 用来返回一个可用服务
- 其实现类 =ServiceProvicer= 有 =GetService/GetRequiredService/CreateScope= 等方法
